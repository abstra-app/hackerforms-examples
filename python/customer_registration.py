...
registration = read_multiple_choice("Hello! Before continuing, what would you like to do?",\
[{"label": "Register a new customer", "value":"first_time"},\
{"label":"Update a customer", "value":"update"}])

if registration == "first_time":
  
    customer = Page().read("Name")\
                    .read_email("Email")\
                    .read_dropdown("Legal entity",[{"label": "Physical", "value": "physical"},\
    ...
    name, email, legal_entity, payment_frequency, payment_method,\
    country, created_data = customer.values()
    ...
    customer = Page().read("Name", required=False)\
                    .read_email("Email", required=False)\
                    .read_dropdown("Legal entity",[{"label": "Physical", "value": "physical"},\
                                          {"label": "Juridical", "value": "juridical"}], required=False)\
                    .read_dropdown("Payment Frequency", ["Monthly", "Annual"], required=False)\
                    .read_dropdown("Payment Method", ["Credit card", "Wire Transfer"], required=False)\
                    .read("Country", required=False)\
                    .read_date("Churn date", required=False)\
                    .run("Send")

    name, email, legal_entity, payment_frequency,\
    payment_method, country, churn_date = customer.values()
