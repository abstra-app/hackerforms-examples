...
def list_services():
    ...
    seniority = read_dropdown("What is your seniority?", ["Intern/Trainee", "Junior", "Mid-level", "Senior", "Leader"])
    rate = read_number("What is your hourly rate?")
    availability = read_number("What many available hours do you have per week?")
...
def view_job_cards(endpoint):
        ...
        for record in range(len(data_records)):
            ...
            hours_needed = str(data_records[record]['fields']['Hours needed per week'])
...
def view_job_board():
    ...
    if action == "everything":
        ...
        if filter_choice == "Seniority needed":
            seniority_filter = read_dropdown("What is your seniority?", ["Intern/Trainee", "Junior", "Mid-level", "Senior", "Leader"])
            endpoint = "https://INSERT_FILTERED_API_ENDPOINT_HERE.com"
            view_job_cards(endpoint)

        elif filter_choice == "Hourly rate":
            hourly_rate_filter = read("What's the minimum hourly rate you want?")
            endpoint = "https://INSERT_FILTERED_API_ENDPOINT_HERE.com"
            view_job_cards(endpoint)
...
def list_job():
    ...
    seniority = read_dropdown("What is the seniority needed?", ["Intern/Trainee", "Junior", "Mid-level", "Senior", "Leader"])
...
def view_dev_cards(endpoint):
    ...
    for record in range(len(data_records)):
        ...
        hourly_rate = str(data_records[record]['fields']['Hourly rate'])
        available_hours = str(data_records[record]['fields']['Available hours per week'])
...
def view_dev_board():
    ...
    if action == "everything":
        ...
        if filter_choice == "Seniority":
            seniority_filter = read_dropdown("What is the seniority needed?", ["Intern/Trainee", "Junior", "Mid-level", "Senior", "Leader"])
            endpoint = "https://INSERT_FILTERED_API_ENDPOINT_HERE.com"
            view_dev_cards(endpoint)

        elif filter_choice == "Hourly rate":
            hourly_rate_filter = read("What's the maximum hourly rate you're willing to pay?")
            endpoint = "https://INSERT_FILTERED_API_ENDPOINT_HERE.com"
            view_dev_cards(endpoint)
