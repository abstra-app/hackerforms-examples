...
contacts_filename = datetime.today().strftime('%Y-%m-%d') + "_hubspot_contacts.json"
...
try:
    ...
    properties_name = [*map(lambda x: x["name"], properties_response.json()["results"])]
    properties_type = [*map(lambda x: x["type"], properties_response.json()["results"])]
except:
...
time_to_get_contact = int((datetime.now() - timedelta(days=1)).timestamp()) #in miliseconds
initial_time_to_get_contact = int((datetime.now() - timedelta(days=360)).timestamp())
filter_group = [
    {
        "filters":[
            {
...
while True:
    ...
    try:
        contacts_response = requests.post(contacts_url, data=json.dumps(data), headers=headers)
        contacts_result.extend([*map(lambda x: x["properties"], contacts_response.json()["results"])])
...
def upload_to_aws(bucket_name,folder_name, s3_file):
    ...
    try:
        s3.Bucket(bucket_name).upload_file(s3_file, ("hubspot/" + folder_name +"/" + s3_file))
        return None
