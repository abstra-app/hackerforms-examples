...
columns_name_authors = ["id", "created_at", "updated_at", "deleted_at", "name", "email", "phone"]
columns_name_events = ["id", "created_at", "type", "author_id"]
columns_name_workspaces = ["id", "created_at", "updated_at", "deleted_at", "author_id", "name"]

data = [{"tables_name": "authors", "select_query":query_authors, "columns_name": columns_name_authors},
       {"tables_name": "events", "select_query": query_events , "columns_name": columns_name_events},
       {"tables_name": "workspaces", "select_query": query_workspaces , "columns_name": columns_name_workspaces}]

for d in data:
    df = postgresql_to_dataframe(conn1, d.get('select_query'), d.get('columns_names'))
