...
amount_brl, cofins_brl, csll_brl, irpj_brl, pis_brl = [float(x) for x in invoice_data.values()]

cofins, csll, irpj, pis, invoice_value = tax_calculator(amount_brl, cofins_brl, csll_brl, irpj_brl, pis_brl)

Page().display("Invoice value with taxes: R$ {}".format(invoice_value))\
      .display("Cofins: R$ {}".format(cofins))\
      .display("Csll: R$ {}".format(csll))\
      .display("Irpj: R$ {}".format(irpj))\
      .display("Pis: R$ {}".format(pis))\
      .run("Ok, got it!")
