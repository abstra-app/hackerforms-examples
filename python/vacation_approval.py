...
if not 'AIRTABLE_VACATION_API_KEY' in os.environ:
...
display(name + " from the " + team + " team has requested " + str(days_requested.days) +
        " days of vacation, from " + start_date_requested.strftime("%x") + " to " + end_date_requested.strftime("%x") + ".")

display("They’ve taken " + str(last_vacation) + " days off in the last 12 months and have " +
        str(time_remaining) + " remaining days to request, according to company policy.")
...
if approve == True:
    ...
    calendar_properties = {'dates': calendar_start_date + '/' +
                           calendar_end_date, 'details': 'Enjoy!', 'text': f"{name}'s Vacation"}
